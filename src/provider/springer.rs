...
#[derive(Deserialize)]
struct SprResp {
    records: Vec<SprRecord>,
    result: Option<SprResult>,
}
#[derive(Deserialize)]
struct SprResult {
    total: Option<usize>,
...
struct SprRecord {
    title: Option<String>,
    creators: Option<Vec<SprCreator>>,
    publicationName: Option<String>,
    publicationDate: Option<String>,
    volume: Option<String>,
    number: Option<String>,
    startingPage: Option<String>,
    doi: Option<String>,
    url: Option<SprUrl>,
...
impl Provider for Springer {
    ...
    fn key_env(&self) -> Option<&'static str> {
        Some("SPRINGER_API_KEY")
    }
    async fn search(&self, query: &str, limit: usize) -> Result<SearchResult> {
        let key = self
            .api_key
            .as_deref()
            .ok_or_else(|| anyhow::anyhow!("Springer requires SPRINGER_API_KEY"))?;
        let url = format!(
        ...
        if !resp.status().is_success() {
            anyhow::bail!("Springer {}: {}", resp.status(), resp.text().await?);
        }
        ...
        let entries = d
            ...
            .map(|r| {
                ...
                if let Some(d) = r.publicationDate {
                    if d.len() >= 4 {
    ...
    async fn fetch_by_id(&self, id: &str) -> Result<BibEntry> {
        let key = self
            .api_key
            .as_deref()
            .ok_or_else(|| anyhow::anyhow!("Springer requires API key"))?;
        let doi = id.trim().strip_prefix("https://doi.org/").unwrap_or(id);
        let url = format!(
            "https://api.springernature.com/meta/v2/json?q=doi:{}&api_key={}",
            doi, key
        );
        let resp = super::http_client()
            .get(&url)
        ...
        let r = d
            ...
            .into_iter()
            .next()
            .ok_or_else(|| anyhow::anyhow!("No Springer result"))?;
