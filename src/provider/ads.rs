...
impl Provider for Ads {
    ...
    async fn search(&self, query: &str, limit: usize) -> Result<SearchResult> {
        let key = self
            .api_key
            .as_deref()
            .ok_or_else(|| anyhow::anyhow!("ADS requires ADS_API_KEY"))?;
        let url = format!(
            "https://api.adsabs.harvard.edu/v1/search/query?q={}&rows={}",
            urlencoding(query),
            limit.min(100)
        );
        ...
        if !resp.status().is_success() {
            anyhow::bail!("ADS {}: {}", resp.status(), resp.text().await?);
        }
        ...
        let entries = d
            ...
            .map(|d| {
                ...
                if let Some(dois) = d.doi {
                    if let Some(doi) = dois.into_iter().next() {
