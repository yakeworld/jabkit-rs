...
impl Provider for Acm {
    ...
    async fn search(&self, query: &str, limit: usize) -> Result<SearchResult> {
        let key = self
            .api_key
            .as_deref()
            .ok_or_else(|| anyhow::anyhow!("ACM requires ACM_API_KEY"))?;
        let url = format!(
        ...
        if !resp.status().is_success() {
            anyhow::bail!("ACM {}: {}", resp.status(), resp.text().await?);
        }
        let d: AcmResp = resp.json().await?;
        let records = d.records.unwrap_or_default();
        let entries: Vec<BibEntry> = records
            .into_iter()
            .map(|r| {
