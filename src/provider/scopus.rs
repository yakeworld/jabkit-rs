...
struct ScoResp {
    ...
    search_results: Option<ScoResults>,
}
#[derive(Deserialize)]
struct ScoResults {
    #[serde(rename = "entry")]
...
impl Provider for Scopus {
    ...
    async fn search(&self, query: &str, limit: usize) -> Result<SearchResult> {
        let key = self
            .api_key
            .as_deref()
            .ok_or_else(|| anyhow::anyhow!("Scopus requires SCOPUS_API_KEY"))?;
        let url = format!(
        ...
        if !resp.status().is_success() {
            anyhow::bail!("Scopus {}: {}", resp.status(), resp.text().await?);
        }
        let d: ScoResp = resp.json().await?;
        let sr = d
            .search_results
            .ok_or_else(|| anyhow::anyhow!("Scopus: empty response"))?;
        let total = sr
            .total
            .and_then(|t| t.parse().ok())
            .unwrap_or(sr.entry.len());
        let entries = sr
            .entry
            .into_iter()
            .map(|e| {
                ...
                if let Some(a) = e.author {
                    ...
                    entry.set_field(Field::Author, e.creator.unwrap_or_default());
                }
