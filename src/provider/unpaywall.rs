...
pub struct Unpaywall { email: Option<String> }
impl Unpaywall {
    pub fn new(email: Option<String>) -> Self { Self { email } }
}
...
struct UpwResult {
    ...
    journal_name: Option<String>, publisher: Option<String>,
    genre: Option<String>, is_oa: Option<bool>,
    best_oa_location: Option<UpwLocation>,
}
#[derive(Deserialize)]
struct UpwAuthor { name: Option<String> }
#[derive(Deserialize)]
struct UpwLocation { url_for_pdf: Option<String> }

#[async_trait]
impl Provider for Unpaywall {
    fn name(&self) -> &'static str { "Unpaywall" }
    fn key_env(&self) -> Option<&'static str> { Some("UNPAYWALL_EMAIL") }
    async fn search(&self, query: &str, limit: usize) -> Result<SearchResult> {
        let email = self.email.as_deref().ok_or_else(|| anyhow::anyhow!("Unpaywall requires UNPAYWALL_EMAIL"))?;
        let url = format!(
        ...
        let entries: Vec<BibEntry> = d.results.into_iter().map(|r| {
            ...
            if let Some(y) = r.year { e.set_field(Field::Year, y.to_string()); }
            e.set_field(Field::Publisher, r.publisher.unwrap_or_default());
            if let Some(oaloc) = r.best_oa_location {
                if let Some(pdf) = oaloc.url_for_pdf { e.set_field(Field::Url, pdf); }
            }
            e
        }).collect();
    ...
    async fn fetch_by_id(&self, id: &str) -> Result<BibEntry> {
        ...
        let mut e = BibEntry::new(EntryType::Article);
        if let Some(t) = d.get("title").and_then(|t| t.as_str()) { e.set_field(Field::Title, t.to_string()); }
        if let Some(doi) = d.get("doi").and_then(|d| d.as_str()) { e.set_field(Field::Doi, doi.to_string()); }
        Ok(e)
    }
}
