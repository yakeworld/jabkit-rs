...
pub struct Ieee { api_key: Option<String> }
impl Ieee {
...
struct IeeeResp {
    ...
    #[serde(default)] total_records: Option<String>,
}

#[derive(Deserialize)]
struct IeeeArticle {
    title: Option<String>, authors: Option<IeeeAuthors>,
    publication_title: Option<String>, publication_year: Option<String>,
    volume: Option<String>, issue: Option<String>, start_page: Option<String>,
...
impl Provider for Ieee {
    ...
    async fn search(&self, query: &str, limit: usize) -> Result<SearchResult> {
        let key = self.api_key.as_deref().ok_or_else(|| anyhow::anyhow!("IEEE requires IEEE_API_KEY"))?;
        let url = format!(
