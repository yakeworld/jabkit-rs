...
pub struct Biodiversity { api_key: Option<String> }
impl Biodiversity {
...
macro_rules! stub_provider {
    ($name:ident, $display:literal, $key_env:expr) => {
        ...
        impl Provider for $name {
            ...
            fn key_env(&self) -> Option<&'static str> { $key_env }
...
impl Provider for Biodiversity {
    fn name(&self) -> &'static str { "BiodiversityHL" }
    fn key_env(&self) -> Option<&'static str> { Some("BIODIVERSITY_KEY") }
    async fn search(&self, query: &str, limit: usize) -> Result<SearchResult> {
        let key = self.api_key.as_deref().ok_or_else(|| anyhow::anyhow!("BiodiversityHL requires BIODIVERSITY_KEY"))?;
        let url = format!(
        ...
        let entries: Vec<BibEntry> = v.pointer("/Result").and_then(|r| r.as_array()).map(|arr| {
            arr.iter().filter_map(|item| {
                ...
                let t = item.get("Title").and_then(|t| t.as_str()).unwrap_or_default();
                e.set_field(Field::Title, t.to_string());
                let a = item.get("Author").and_then(|a| a.as_str()).unwrap_or_default();
                e.set_field(Field::Author, a.to_string());
                let y = item.get("Year").and_then(|y| y.as_str()).unwrap_or_default();
    ...
    async fn fetch_by_id(&self, _id: &str) -> Result<BibEntry> { anyhow::bail!("BiodiversityHL ID lookup not supported") }
}
